...
// Check if a visit has been recorded in the current session
if (!isset($_SESSION['last_visit'])) {
    $_SESSION['last_visit'] = time();

    // Save the view count in the database
    $req = $db->prepare('UPDATE visites SET nombres = nombres + 1');
    $req->execute();

    // Get the updated view count
    $visits = (int) $db->query('SELECT nombres FROM visites')->fetchColumn();
} else {
    // Get the updated view count
    $visits = (int) $db->query('SELECT nombres FROM visites')->fetchColumn();
...
echo "Cette page a été visitée $visits fois.";
